...
def train(batch_num, velodyne_path, label_path=None, calib_path=None, resolution=0.2, dataformat="pcd", label_type="txt", is_velo_cam=False, scale=4, voxel_shape=(800, 800, 40)):
    # tf Graph input
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            for (batch_x, batch_g_map, batch_g_cord) in lidar_generator(batch_num, velodyne_path, label_path=label_path, \
               calib_path=calib_path,resolution=resolution, dataformat=dataformat, label_type=label_type, is_velo_cam=is_velo_cam, scale=scale):
            ...
            if (epoch != 0) and (epoch % 10 == 0):
                ...
                saver.save(sess, "velodyne_11th_try_" + str(epoch) + ".ckpt")
        print("Optimization Finished!")

def test(batch_num, velodyne_path, label_path=None, calib_path=None, resolution=0.2, dataformat="pcd", label_type="txt", is_velo_cam=False, scale=4, voxel_shape=(800, 800, 40)):
    # tf Graph input
    ...
    voxel =  raw_to_voxel(pc, resolution=resolution, x=(0, 80), y=(-50, 50), z=(-2.5, 2.0))
    ...
    with tf.Session() as sess:
        model, voxel, phase_train = ssd_test_model(sess, voxel_shape=(900, 1000, 100), activation=tf.nn.relu)
        # optimizer = create_optimizer(total_loss)
        saver = tf.train.Saver()
        new_saver = tf.train.import_meta_graph("velodyne_11th_try_100.ckpt.meta")
        # last_model = tf.train.latest_checkpoint('./velodyne_11th_try_100.ckpt')
        last_model = "./velodyne_11th_try_90.ckpt"
        print last_model
...
def lidar_generator(batch_num, velodyne_path, label_path=None, calib_path=None, resolution=0.2, dataformat="pcd", label_type="txt", is_velo_cam=False, scale=4):
...
if __name__ == '__main__':
    ...
    # calib_path = "../data/training/calib/007000.txt"
