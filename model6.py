...
def conv3DLayer(input_layer, input_dim, output_dim, height, width, length, stride, activation=tf.nn.relu, padding="SAME", name="", is_training=True):
    ...
    with tf.variable_scope("conv3D" + name) as c3:
        ...
        # bias = tf.contrib.layers.batch_norm(bias, data_format='NHWC', center=True, scale=True, is_training=is_training, scope=c3)
    return bias
...
def conv3DLayer_test(input_layer, input_dim, output_dim, height, width, length, stride, activation=tf.nn.relu, padding="SAME", name="", is_training=True):
    ...
    with tf.variable_scope("conv3D" + name) as c3:
        ...
        if activation:
            bias = activation(bias, name="activation")
        # bias = tf.contrib.layers.batch_norm(bias, data_format='NHWC', center=True, scale=True, is_training=is_training, scope=c3, reuse=True)
    return bias
...
class BNBLayer(object):
    ...
    def build_graph(self, voxel, activation=tf.nn.relu, is_training=True):
        ...
        # self.layer1 = tf.contrib.layers.batch_norm(self.layer1, data_format='NHWC', center=True, scale=True, is_training=is_training, name="norm1")
        self.layer2 = conv3DLayer(self.layer1, 10, 20, 5, 5, 5, [1, 2, 2, 2, 1], name="layer2", activation=activation, is_training=is_training)
        # self.layer2 = tf.contrib.layers.batch_norm(self.layer2, data_format='NHWC', center=True, scale=True, is_training=is_training, name="norm2")
        self.layer3 = conv3DLayer(self.layer2, 20, 30, 3, 3, 3, [1, 2, 2, 2, 1], name="layer3", activation=activation, is_training=is_training)
        # self.layer3 = tf.contrib.layers.batch_norm(self.layer3, data_format='NHWC', center=True, scale=True, is_training=is_training, name="norm3")
    ...
    def build_test_graph(self, voxel, activation=tf.nn.relu, is_training=True):
        self.layer1 = conv3DLayer_test(voxel, 1, 10, 5, 5, 5, [1, 2, 2, 2, 1], name="layer1", activation=activation, is_training=is_training)
        self.layer2 = conv3DLayer_test(self.layer1, 10, 20, 5, 5, 5, [1, 2, 2, 2, 1], name="layer2", activation=activation, is_training=is_training)
        self.layer3 = conv3DLayer_test(self.layer2, 20, 30, 3, 3, 3, [1, 2, 2, 2, 1], name="layer3", activation=activation, is_training=is_training)
        base_shape = self.layer2.get_shape().as_list()
        obj_output_shape = [tf.shape(self.layer3)[0], base_shape[1], base_shape[2], base_shape[3], 2]
        cord_output_shape = [tf.shape(self.layer3)[0], base_shape[1], base_shape[2], base_shape[3], 24]
        self.objectness = deconv3D_to_output(self.layer3, 30, 2, 3, 3, 3, [1, 2, 2, 2, 1], obj_output_shape, name="objectness", activation=None)
...
def ssd_model(sess, voxel_shape=(300, 300, 300),activation=tf.nn.relu, is_training=True):
    ...
    with tf.variable_scope("3D_CNN_model") as scope:
        bnb_model = BNBLayer()
        bnb_model.build_graph(voxel, activation=activation, is_training=is_training)
...
def ssd_test_model(sess, voxel_shape=(300, 300, 300),activation=tf.nn.relu, is_training=True):
    voxel = tf.placeholder(tf.float32, [None, voxel_shape[0], voxel_shape[1], voxel_shape[2], 1])
    with tf.variable_scope("3D_CNN_model") as scope:
        bnb_model = BNBLayer()
        bnb_model.build_test_graph(voxel, activation=activation, is_training=is_training)
...
def train(batch_num, velodyne_path, label_path=None, calib_path=None, resolution=0.2, dataformat="pcd", label_type="txt", is_velo_cam=False):
    ...
    with tf.Session() as sess:
        model, voxel = ssd_model(sess, voxel_shape=(360, 400, 40), activation=tf.nn.relu, is_training=True)
        saver = tf.train.Saver()
        total_loss, obj_loss, cord_loss, g_map, g_cord, y = loss_func3(model)
        optimizer = create_optimizer(total_loss, lr=0.005)
...
def test(batch_num, velodyne_path, label_path=None, calib_path=None, resolution=0.2, dataformat="pcd", label_type="txt", is_velo_cam=False):
    ...
    with tf.Session() as sess:
        model, voxel = ssd_test_model(sess, voxel_shape=(360, 400, 40), activation=tf.nn.relu, is_training=False)
