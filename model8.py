...
def loss_func(model):
    ...
    obj_loss = tf.reduce_sum(-tf.log(tf.div(sum_object_loss, bunbo)))
    ...
    return  obj_loss, obj_loss, cord_loss, obj_loss, obj_loss, g_map, g_cord, y
...
def loss_func3(model):
    ...
    non_obj_loss = -tf.reduce_sum(tf.multiply(non_gmap, tf.log(y[:, :, :, :, 1] + elosion)))
    cross_entropy = tf.add(is_obj_loss, tf.multiply(non_obj_loss, 0.0001))
    obj_loss = cross_entropy

    g_cord = tf.placeholder(tf.float32, model.cordinate.get_shape().as_list())
    cord_diff = tf.multiply(g_map, tf.reduce_sum(tf.square(tf.subtract(model.cordinate, g_cord)), 4))
    cord_loss = tf.multiply(tf.reduce_sum(cord_diff), 0.01)
    return obj_loss, obj_loss, cord_loss, is_obj_loss, non_obj_loss, g_map, g_cord, y
...
def train(batch_num, velodyne_path, label_path=None, calib_path=None, resolution=0.2, dataformat="pcd", label_type="txt", is_velo_cam=False):
    ...
    with tf.Session() as sess:
        ...
        total_loss, obj_loss, cord_loss, is_obj_loss, non_obj_loss, g_map, g_cord, y = loss_func(model)
        ...
        for epoch in range(training_epochs):
            ...
            if (epoch != 0) and (epoch % 5 == 0):
                ...
                saver.save(sess, "velodyne_8th_try_" + str(epoch) + ".ckpt")
...
def test(batch_num, velodyne_path, label_path=None, calib_path=None, resolution=0.2, dataformat="pcd", label_type="txt", is_velo_cam=False):
    ...
    with tf.Session() as sess:
        ...
        saver = tf.train.Saver()
        new_saver = tf.train.import_meta_graph("velodyne_8th_try_5.ckpt.meta")
...
if __name__ == '__main__':
    ...
    #
    # pcd_path = "../data/training/velodyne/000300.bin"
    # label_path = "../data/training/label_2/000300.txt"
    # calib_path = "../data/training/calib/000300.txt"
